...
#' @param result The decision matrix (\emph{m} x (\emph{n}*3)) with the values of the \emph{m} alternatives, for the \emph{n} criteria, and multiplied by 3 since they are triangular fuzzy numbers.
...
)
{
  ...
  if(! is.matrix(result))
    stop("'result' must be a matrix with the values of the alternatives")
  if(missing(weights))
  ...
  new_cb <- c(1:ncol(result))
  k=1
  for(j in seq(1, ncol(result), 3)){
    if (cb[k] == 'max'){
      new_cb[j] <- 'max'
      new_cb[j+1] <- 'max'
      new_cb[j+2] <- 'max'
  ...
  #1. Ideal solutions
  posI <- as.integer(new_cb == "max") * apply(result, 2, max) +
    as.integer(new_cb == "min") * apply(result, 2, min)
  negI <- as.integer(new_cb == "min") * apply(result, 2, max) +
    as.integer(new_cb == "max") * apply(result, 2, min)

  #2. S and R index
  #Normalization
  d <- matrix(nrow = nrow(result), ncol = ncol(result))
  for(i in seq(1, ncol(result), 3)){
    if (new_cb[i] == 'max'){
      denominator = posI[i+2]-negI[i]
      d[,i] = (posI[i] - result[,i+2])/denominator
      d[,i+1] = (posI[i+1] - result[,i+1])/denominator
      d[,i+2] = (posI[i+2] - result[,i])/denominator
    }
    else{
      denominator = negI[i+2]-posI[i]
      d[,i] = (result[,i] - posI[i+2])/denominator
      d[,i+1] = (result[,i+1] - posI[i+1])/denominator
      d[,i+2] = (result[,i+2] - posI[i])/denominator
    }
  }
  ...
  S <- matrix(nrow = nrow(result), ncol = 3)
  R <- matrix(nrow = nrow(result), ncol = 3)

  S[,1] <- apply(NW[,seq(1, ncol(result), 3)], 1, sum)
  S[,2] <- apply(NW[,seq(2, ncol(result), 3)], 1, sum)
  S[,3] <- apply(NW[,seq(3, ncol(result), 3)], 1, sum)

  R[,1] <- apply(NW[,seq(1, ncol(result), 3)], 1, max)
  R[,2] <- apply(NW[,seq(2, ncol(result), 3)], 1, max)
  R[,3] <- apply(NW[,seq(3, ncol(result), 3)], 1, max)


  #3. Q index
  Q1 <- matrix(nrow = nrow(result), ncol = 3)
  Q2 <- matrix(nrow = nrow(result), ncol = 3)
  ...
  return(data.frame(Alternatives = 1:nrow(result), S = S, Def_S = Def_S, R = R, Def_R = Def_R, Q = Q, Def_Q = Def_Q, Ranking = rank(Def_Q, ties.method= "first")))
